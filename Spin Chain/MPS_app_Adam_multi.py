...
Net, no = 'Deep_sig_Mix', ['002', '003', '004', '005', '006']
id = ['10']
amount, epochs, iter_times, processors = 50, 1, 250, 10
init_no, init_amount, keep_amount = 0, 20, 4
tol, delta = 5e-3, 5e-2 # usually tol=1e-3
...
eta = 1e3/1.5
class optimization():
    def __init__(self, model, J, g, a, tol):
    ...
    def __call__(self, init):
        ...
        LH = LagMPS_multi(self.model, init, N, init_penalty, 'periodic').to(device)
        ...
        if LH.P.item() < self.tol:
            return (LH.H.item(), LH.s[0, 2].item(), LH.P.item(), LH.Eps.item(), LH.s.data.cpu().numpy())
        else:
            return (LH.H.item(),)
...
def plot_g(g, y, y_label):
    ...
    plt.xlabel('g')
    plt.ylabel(y_label)
    plt.savefig(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/{}.png'.format(L, opt, Net, id[-1], y_label))
    plt.close()
...
def plot_Hs(g, Hs):
    ...
    plt.xlabel('g')
    plt.ylabel('Hs')
    plt.savefig(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/Hs.png'.format(L, opt, Net, id[-1]))
    plt.close()
...
def main():
    ...
    g = 0. + 0.02 * np.arange(1, amount + 1)
    a = 0.25

    print('Eta:\t', eta, file=f)

    print('Spin-1/2 chain at thermodynamic limit, LT Ising model', file=f)
    ...
    for i in range(amount):
        coefficients = torch.tensor([1., g[i], a]).to(device) # [J, g, a]
        while True:
            ...
            totData = pool.map(optimization(model=model, J=coefficients[0], g=coefficients[1], a=coefficients[2], tol=tol), totInits)
            pool.close()
            pool.join()
            for data in totData:  # data:[LH.H.item(), LH.s[0, 2].item(), LH.P.item(), LH.Eps.item(), LH.s.data.cpu().numpy()]
                Hs[i] += [data[0]]
                if len(data) > 1:
    ...
    plot_g(g, OP, 'OP')
    plot_Hs(g, Hs)
    np.save(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/s.npy'.format(L, opt, Net, id[-1]), s)
    np.save(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/E.npy'.format(L, opt, Net, id[-1]), Hmin)
    np.save(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/P.npy'.format(L, opt, Net, id[-1]), P)
    np.save(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/Es.npy'.format(L, opt, Net, id[-1]), Hs)


if __name__ == "__main__":
    if not os.path.exists(DefaultPath + 'OUT/mps_app/L={}/{}'.format(L, opt)):
        os.mkdir(DefaultPath + 'OUT/mps_app/L={}/{}'.format(L, opt))
    if not os.path.exists(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi'.format(L, opt, Net, id[-1])):
        os.mkdir(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi'.format(L, opt, Net, id[-1]))
    ...
    f = open(DefaultPath + 'OUT/mps_app/L={}/{}/{}_{}_multi/{}_{}_multi.txt'.format(L, opt, Net, id[-1], Net, id[-1]), 'w+')
